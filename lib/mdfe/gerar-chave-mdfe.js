...
function gerarCodigoNumericoMDFe(cMDF) {
  if (cMDF === undefined || cMDF === null || String(cMDF).trim() === "") {
    return String(Math.floor(Math.random() * 100000000)).padStart(8, "0");
  }

  const codigo = String(cMDF).trim();

  if (!/^\d{8}$/.test(codigo)) {
    throw new Error("cMDF deve conter 8 dígitos numéricos.");
  }
...
function gerarChaveMDFe({
  ...
  const modelo = "58";
  const codigoMDF = gerarCodigoNumericoMDFe(cMDF);
